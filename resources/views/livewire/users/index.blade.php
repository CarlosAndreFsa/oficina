<div>
    ...
    <x-header title="Customers" separator progress-indicator>
        ...
        <x-slot:actions>
            ...
            <x-button label="Create" icon="o-plus" link="{{ route('users.create') }}" class="btn-primary" responsive />
        </x-slot:actions>
    </x-header>

    {{--  TABLE --}}
    <x-card>
        <x-table :headers="$this->headers" :rows="$this->users" :sort-by="$sortBy" link="/users/{id}" with-pagination>
            {{-- Avatar scope --}}
