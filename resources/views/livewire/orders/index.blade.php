<div>
    ...
    {{-- TABLE --}}
    <x-card shadow>
        <x-table :headers="$this->headers" :rows="$this->orders" link="/orders/{id}/edit" with-pagination :sort-by="$sortBy">
    ...
    <x-drawer wire:model="showFilters" title="Filters" class="lg:w-1/3" right separator with-close-button>
        <div class="grid gap-5" @keydown.enter="$wire.showFilters = false">
            ...
            <x-select label="Status" :options="$this->statuses" wire:model.live="status_id" icon="o-map-pin" placeholder="All" placeholder-value="0" inline />
            <x-select label="Country" :options="$this->countries" wire:model.live="country_id" icon="o-flag" placeholder="All" placeholder-value="0" inline />
        </div>
